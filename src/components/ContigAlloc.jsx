...
const ContigAlloc = (props) => {
  ...
  useEffect(() => {
    if (compactionEnabled) {
      const updatedPdtData = allocationData.map((process) => {
        ...
        return {
          ...pdtEntry,
          base: pdtData.indexOf(pdtEntry) + 1,
        };
      });
      const updatedAllocationData = allocationData.map((process) => {
        const allocationEntry = allocationData.find(
          (entry) => entry.id === process.id
        );
        return {
          ...allocationEntry,
          allocation: allocationData.indexOf(allocationEntry) + 1,
        };
      });
    ...
    if (partitionType === "dynamic" && !isNaN(newSize) && newSize > 0) {
      if (totalAllocatedSize + newSize <= props.memorySize) {
        ...
        if (base !== -1) {
          ...
          alert("No suitable memory block found for the process.");
        }
      } else {
  ...
  const findBestFitBaseForProcess = (size) => {
    ...
    return bestFitBase !== -1 ? bestFitBase : -1;
  };
  ...
  const findWorstFitBaseForProcess = (size) => {
    ...
    return worstFitBase !== -1 ? worstFitBase : -1;
  };

  let nextFitLastIndex = 0;
  const findNextFitBaseForProcess = (size) => {
    for (let i = nextFitLastIndex; i < memoryBlocksData.length; i++) {
      if (memoryBlocksData[i] === "transparent") {
        let count = 0;
        let base = i;
        while (
    ...
    nextFitLastIndex = 0;
    return findNextFitBaseForProcess(size);
  };
        ...
        <div className="border-r-black border sm:border-b-black sm:border-r-0 w-1/2 flex flex-col h-[525px] sm:w-full bg-[#C5FAD5]">
          <div className="w-full flex-col flex items-center justify-center h-[350px]">
            <div className="p-2 items-center justify-center flex">
              <label htmlFor="partitionType">Select Partition Type :</label>
              <select
                id="partitionType"
                name="partitionType"
            ...
            {partitionType === "static" && (
              <>
                ...
                <label htmlFor="selectedStrategy">
                  <input
                    type="radio"
                    id="selectedStrategy"
                          ...
                          {allocationData.map((entry, index) => (
                            <div
                              ...
                              style={{
                                ...
                                height: `${
                                  (entry.size / props.memorySize) * 100
                                }%`,
                                width: "100%",
                                border: "none",
                              }}
