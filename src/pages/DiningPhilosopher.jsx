...
const DiningPhilosopher = () => {
        ...
        newPhilosophers.forEach((status, index) => {
            ...
            switch (status) {
                case statuses.THINKING:
                    if (randomChance > 0.5) { // 50% chance to become HUNGRY
                        newPhilosophers[index] = statuses.HUNGRY;
                    }
                    break;
                case statuses.HUNGRY:
                    if (newPhilosophers[left] !== statuses.EATING && newPhilosophers[right] !== statuses.EATING) {
                        newPhilosophers[index] = statuses.EATING;
                    }
                    break;
                case statuses.EATING:
                    newPhilosophers[index] = statuses.THINKING;
                    break;
            }
        });
    ...
    const getStatusColor = (status) => {
        switch (status) {
            case statuses.THINKING: return 'bg-red-500';
            case statuses.EATING: return 'bg-blue-500';
            case statuses.HUNGRY: return 'bg-green-500';
            case statuses.GRABBING_RIGHT: case statuses.GRABBING_LEFT: return 'bg-yellow-500';
            default: return '';
        }
    };
    
    const getChopstickColor = (leftPhilosopherStatus, rightPhilosopherStatus) => {
        if(leftPhilosopherStatus === statuses.EATING && rightPhilosopherStatus === statuses.EATING) {
            return 'bg-purple-500';
        } else if(leftPhilosopherStatus !== statuses.EATING && rightPhilosopherStatus !== statuses.EATING) {
            return 'bg-teal-500';
        } else {
            return 'bg-gray-500';
        }
    ...
    return (
        <React.Fragment key={index}>
            ...
            <div
                ...
                className={`absolute w-4 h-4 ${getChopstickColor(philosophers[left], status)} flex items-center justify-center`}
            >
                C
            </div>
        </React.Fragment>
            ...
            <div className='w-3/12 h-full'>
                ...
                <div className='h-2/5 bg-slate-200 text-sm'>
                    ...
                    <div className="flex items-center mb-1"><div className="w-4 h-4 rounded-full bg-red-500 mr-2"></div><p>PHILOSOPHER THINKING</p></div>
                        <div className="flex items-center mb-1"><div className="w-4 h-4 rounded-full bg-blue-500 mr-2"></div><p>PHILOSOPHER EATING</p></div>
                        <div className="flex items-center mb-1"><div className="w-4 h-4 rounded-full bg-yellow-500 mr-2"></div><p>PHILOSOPHER WAITING</p></div>
                        <div className="flex items-center mb-1"><div className="w-4 h-4 rounded-full bg-green-500 mr-2"></div><p>PHILOSOPHER HUNGRY</p></div>
                        <div className="flex items-center mb-1"><div className="w-4 h-4  bg-purple-500 mr-2"></div><p>CHOPSTICK ACQUIRED FOR EATING</p></div>
                        <div className="flex items-center mb-1"><div className="w-4 h-4  bg-teal-500 mr-2"></div><p>ONE CHOPSTICK ACQUIRED</p></div>
                        <div className="flex items-center"><div className="w-4 h-4  bg-gray-500 mr-2"></div><p>CHOPSTICK NOT ACQUIRED</p></div>
